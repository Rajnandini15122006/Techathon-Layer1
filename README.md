...
- **POST /risk/compute-hrvc** - Compute HRVC risk scores for all cells
- **GET /risk/high-risk-cells** - Get high-risk cells above threshold
...
- ✅ **Layer 3: HRVC Risk Engine** - Risk = (Hazard × Exposure × Vulnerability) ÷ Capacity
- ✅ **Ward Priority List** - Automated risk-based ward ranking
- ✅ **Interactive Risk Dashboard** - Real-time risk visualization
- ✅ **Risk-based visualization** (Minimal/Low/Medium/High/Critical)
- 🔄 Resource prepositioning (Layer 4 - coming soon)
- 🔄 Alert scheduling (Layer 5 - coming soon)

## 🟡 Layer 3: HRVC Risk Engine

The HRVC Risk Engine calculates flood risk scores for each grid cell using a simplified formula:
...
- Or visit http://localhost:8000/static/risk_dashboard.html

### API Endpoints

```bash
# Calculate risk scores for an area
curl "http://localhost:8000/api/risk/calculate?lat_min=18.45&lat_max=18.55&lon_min=73.80&lon_max=73.90"

# Get ward priorities sorted by risk
curl "http://localhost:8000/api/risk/ward-priorities?lat_min=18.45&lat_max=18.55&lon_min=73.80&lon_max=73.90"
```

### Risk Components

- **Hazard**: Rainfall forecast, river level, soil saturation
- **Exposure**: Population density, traffic density
- **Vulnerability**: Slum %, elderly %, low elevation %
- **Capacity**: Shelter count, hospital beds, drain strength

### Ward Priority List

The ward priority list is automatically generated by aggregating grid cell risks by ward and sorting by average risk score. No separate module needed - it's a dashboard feature!

### Documentation

- **[README_LAYER3.md](README_LAYER3.md)** - Quick start guide for Layer 3
- **[LAYER3_RISK_ENGINE.md](LAYER3_RISK_ENGINE.md)** - Complete technical documentation
